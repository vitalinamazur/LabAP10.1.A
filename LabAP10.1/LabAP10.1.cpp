...
double PercentExcellent(const Student* students, const int N);
Student GetBestStudent(const Student* students, const int N);
...
int main() {
    ...
    cout << "Студент з найвищим середнім балом: " << bestStudent.prizv << endl;

    double percent = PercentExcellent(students, N);
    cout << "Процент студентів, які вчаться на «відмінно» (оцінка 5 з усіх предметів): "
...
double PercentExcellent(const Student* students, const int N) {
    ...
    for (int i = 0; i < N; i++) {
        // Перевірка на "відмінно"
        if (students[i].fizyka == 5 && students[i].matematika == 5 && students[i].informatyka == 5) {
            count++;
        }
        total++;
    }
...
Student GetBestStudent(const Student* students, const int N) {
    ...
    double maxAvg = (students[0].fizyka + students[0].matematika + students[0].informatyka) / 3.0;
